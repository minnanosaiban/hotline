...
- **並びは「判決文の答え合わせ → 裁判所 → 被告（ＥＮＥＯＳ）側 → 原告（通報者）側」**（各欄の見出しは `<p class="t-toc-head bar-title doc-side">`）。
  **「判決文の答え合わせ」**（2026-09-21 全面改稿）は、判決文の記述と、同じ訴訟記録の中の資料とを突き合わせる欄で、**10の行**に分けてある。どれも書面ではないので、PDF・`.md`・要約のボタンは付けない `data-plain` の行。並びは:
  「この欄の読み方――何と何を照合しているか」（`bunseki-yomikata`）、「照合０　高裁は、地裁の理由を「補強」したのか「差し替えた」のか」（`bunseki-sashikae`）、「理由１　契約書の問題を指摘する通報は、そもそも無かった」（`bunseki-riyu-1`）、「理由２　「海外消費税を支払う合意」をしていたと推認できる」（`bunseki-riyu-2`）、「被告（ＥＮＥＯＳ）の主張のしかたは、勝訴の決め手だったのか」（`bunseki-senryaku`）、「判決文の言い回しが、読み手に与える影響」（`bunseki-iimawashi`）、「正当と評価できる部分」（`bunseki-seito`）、「判決文の認定事実に基づく事実のタイムライン」（`bunseki-timeline`）、「関係法令・規程」（`bunseki-horei`）、「判決文の全文」（`bunseki-zenbun`。判決文の行へ案内する短い文）。本文は、公開しない取り込み用の `parts/bunseki-*.md`（`--8<--` で取り込む）。
...
- `parts/bunseki-*.md`（10本）: 「判決文の答え合わせ」の各行の本文。公開されない（`docs/` の外）取り込み用。`.md` ボタンの元にする書面の本文は `docs/trial/md/` に置く
