...
株価分析（blog）は別サイトへ分離する前提なので、ここには持ち込んでいない。元の `hotline` リポジトリには一切触れていない。

## 元の hotline との違い

- **株価分析（blog/）を除いた**。nav・フックのブログ分岐・`docs/blog/` がなくなった
- **右側の目次（Toc）を全ページで出さない**（front matter の `hide: toc`）。目次の自動退避 JS（`toc-toggle.js`）と CSS は削除した
...
- 音声ファイル（`docs/img/*.m4a`・`*.wav`、約100MB）と `deploy.bat` は持ち込んでいない
...
`doc_indent.py` は不要**になる（`:include:` は、mkdocs.yml で有効な `pymdownx.snippets` の `--8<--` に置き換えられる）。

注意: 貼った本文の `##` などの見出しは、この構成では目次に出ない（目次を出さないため）。書面の中では、見出しの代わりに番号付きの段落を使う。

## 未対応・要判断

- **ホスティング・URL**: `site_url` と各ページの `url:`／`image:`（OGP）は、現行の `https://minnanosaiban.github.io/hotline/` のまま
- **音声解説**（NotebookLM）: 本文中のリンクは現行サイトの音声ファイルを指している。現行サイトを消すと聞こえなくなるので、移す先を決める
- `docs/about/index.md` の株価分析の記事リンク3件は、仮に現行のブログURLを指している。分離後の URL に直す
- `10-blog-reset.css` にブログ専用のルールが残っている（ブログ以外の見出し・表の共通ルールも同居しているため、未整理）
