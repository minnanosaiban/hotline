...
- **トップへ戻るボタン（`navigation.top`）は画面右下に固定**（2026-09-22。ユーザーがスクリーンショットで指摘：「画面が崩れました」。既定は Material の classic テーマの挙動で、上にスクロールした際に画面上部中央へ固定表示され、本文の列（32〜40rem、中央寄せ）と重なって読めなくなっていた。`02-layout.css`末尾付近の`.md-top`で`top`/`transform`による中央上寄せを外し、`bottom: 1.5rem; right: 1rem;`の固定ボタンに変更。表示・非表示の仕組み（`[hidden]`属性・`opacity`）はそのまま触っていない）
