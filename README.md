...
- **並びは「判決文の答え合わせ → 裁判所 → 被告（ＥＮＥＯＳ）側 → 原告（通報者）側」**（各欄の見出しは `<p class="t-toc-head bar-title doc-side">`）。
  ...
  **改稿の趣旨（AI にも人にも誤読されない形にする）**: 元の `理由１`・`理由２` は、長い地の文の中に反論が4つ・3つ並ぶだけで、いま何番目の話を読んでいるのか分からなかった。そこで、**1つの論点＝1枚の「照合カード」（`.ac-card`）**にし、どのカードも同じ順の段で書く――
  ...
  **「ここから先は言えない」は全カードに必ず置く。** 読み手（人も AI も）は言い切られているかを本当かどうかの代用にするので、上限を自分で書いておくことが誤読対策になる。元になった分析（`G:\マイドライブ\eneos_pj-01_ENEOS新訴枠組み_前訴判決の誤読防止ガイド.md`）の「守れる線／崩れる線」をそのまま画面に出す形。
  ...
  照合カードから判決文へ飛ぶために、判決文側にアンカーを3つ足した（2026-09-21）: `2025-kousai-judgment_sashikae`（高裁「原判決１０頁５行目冒頭から…次のとおり改める」）、`2025-tisai-judgment_kozo`（地裁「通報者個人のために設けられたものではない」）、`2025-tisai-judgment_mushikaeshi`（地裁「信義則に反するとまでいうことはできない」）。
