...
- **トップへ戻るボタン（`navigation.top`）は廃止**（2026-09-22。ユーザーがスクリーンショットで指摘：「画面が崩れました」。既定は Material の classic テーマの挙動で、上にスクロールした際に画面上部中央へ固定表示され、本文の列（32〜40rem、中央寄せ）と重なって読めなくなっていた。まず`02-layout.css`の`.md-top`で`bottom: 1.5rem; right: 1rem;`の右下固定に変えたところ、今度は裁判文書ページのサイドノート（`.sn-note`。本文列の右余白に出る。右端の余白は同じく1rem）と衝突した。位置をどこに動かしても本文かサイドノートのどちらかと当たりうるため、ユーザーの判断で**機能ごと廃止**した：`mkdocs.yml`の`theme.features`から`navigation.top`を削除（要素自体が描画されなくなる）、`02-layout.css`の`.md-top`上書きも削除（不要になったため）。同機能はもともと`# - navigation.top`として無効リストにも重複していたので、有効リストから外しただけで元の「未使用」設定に戻る形）
...
- **並びは「高裁判決の根拠 → 裁判所 → 被告（ＥＮＥＯＳ）側 → 原告（通報者）側」**（各欄の見出しは `<p class="t-toc-head bar-title doc-side">`）。
  **「高裁判決の根拠」**（2026-09-22 に「判決文の答え合わせ」から改名。2026-09-21 全面改稿）は、判決文の記述と、同じ訴訟記録の中の資料とを突き合わせる欄。どれも書面ではないので、PDF・`.md`・要約のボタンは付けない `data-plain` の行。本文は、公開しない取り込み用の `parts/bunseki-*.md`（`--8<--` で取り込む）。
  **2026-09-22（その4）：ユーザーの指摘「分量が多すぎるので、この点に焦点をあててその他は削除します」で、9行から2行（`bunseki-riyu-1`・`bunseki-riyu-2`）に削減。** 対象は前訴の核心２点――「理由１＝契約書の問題がはっきりしたのは令和元年12月だとする部分」「理由２＝海外消費税を支払う合意をしていたと推認する部分」（元の分析：`G:\マイドライブ\eneos_pj\02-01_ENEOS新訴枠組み\03_前訴判決の誤読防止ガイド.md`）。`bunseki-riyu-1` は照合１（令和元年12月起点論）だけを残し、照合２〜４（「整理されない形」の中身・「対象を特定した上で」基準・原告の反論への不応答＝いずれも別論点の「対象特定」基準の話）を削除。`bunseki-riyu-2` は１つの結論（合意の推認）を照合１〜４で多角的に検証する構成のため、全体を残した。削除したのは行ごと：`bunseki-senryaku`（ＥＮＥＯＳの主張の採否）・`bunseki-yomikata`（欄の読み方）・`bunseki-iimawashi`（言い回しの影響）・`bunseki-seito`（正当と評価できる部分）・`bunseki-timeline`（事実のタイムライン）・`bunseki-horei`（関係法令・規程）・`bunseki-zenbun`（判決文の全文。全文自体は下の「裁判所」欄の `kousai` 行に残る）。対応する `parts/bunseki-*.md` 7本はファイルごと削除した（内容は git 履歴に残る）。
  **2026-09-22（その5）**：ユーザーの指摘「『対象を特定した上で』という判断も重要でした。規程も指針もそのようなことは求めていません」で、その4で削った旧照合３相当を`bunseki-riyu-1`の照合２として復元（削っていた旧照合４は復元せず、照合３の論点だけを対象特定論に絞って統合）。もとの照合３より根拠を強化：①本件規程１．２⑸（甲１＝`G:\マイドライブ\eneos_pj\01-04_前訴裁判資料_R6ワ24718\K0101_甲01_本件規程.pdf`）と消費者庁ハンドブックＱ６（甲３２＝同フォルダの`K3201_甲32_公益通報ハンドブック_21頁.pdf`）を実際に読んでＰＤＦと一致することを確認した上で引用、②原告第５準備書面（`#dai5_261`）に加え、**控訴審の控訴理由書で同じ主張が繰り返された箇所に、被告が「認める」と答えていた**事実を追加（`docs/trial/md/kouso.md.txt` 574〜583行目。574行目に新アンカー`kouso_224_tokutei`を追加）。判決の「対象を特定した上で」という基準が、被告自身も争っていない前提に触れないまま採用されている、という点が核心。
  **2026-09-22（その6）**：ユーザーの提案「現状、情報量が多すぎて読みにくいので、アコーディオンは以下の３つにするのはいかがですか」で、その4で削った`bunseki-senryaku`（ＥＮＥＯＳの主張の採否）を git 履歴から復元し、３行構成に戻した：①「ＥＮＥＯＳの主張のなかで、判決に採用されたのはどれか」（`bunseki-senryaku`。中身は変更なし。もともと「答え」＋箇条書き＋言えること／ここから先は言えないだけが常に見え、４つの照合は`<details class="ac-more">`で折りたたみ済み＝その2の対応がそのまま効いている）②「なぜ、契約書の問題を指摘する通報は、そもそも無かったとされたか」（`bunseki-riyu-1`。旧題「理由１　契約書の問題を指摘する通報は、そもそも無かった」から、ユーザー指定の「なぜ…とされたか」疑問形に改題）③「なぜ、『海外消費税を支払う合意』をしていたと推認することとなったか」（`bunseki-riyu-2`。旧題「理由２　『海外消費税を支払う合意』をしていたと推認できる」から同様に改題）。**改題したのはアコーディオンの`<summary>`（一覧に見える行の見出し）だけ**で、各行の中の`.f-head`（「理由１」「理由２」のラベルと本来の言い切り形タイトル）はそのまま。`bunseki-senryaku`のサマリーは旧題「ＥＮＥＯＳの主張は、どこまで判決に採用されたか」からユーザー指定の「ＥＮＥＯＳの主張のなかで、判決に採用されたのはどれか」に変更。
