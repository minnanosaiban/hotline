...
def make_section_distribution() -> None:
    ...
    ax.invert_yaxis()
    ax.set_xlabel("ルール数", fontsize=10.5, color=C_TEXT_SUB)
    ...
    fig.text(0.95, -0.02,
             ...
             fontsize=9.5, color=C_TEXT_SUB, ha="right", style="italic")
...
def make_standard_category_heatmap() -> None:
    ...
    ax.set_yticklabels(cross.index, fontsize=10.5)
    ...
    for i, total in enumerate(cross.sum(axis=1).values):
        ax.text(len(cols) - 0.2, i, f"計 {total}", fontsize=10,
                ha="left", va="center", color=C_TEXT_SUB,
                fontweight="bold", clip_on=False)

    cbar = plt.colorbar(im, ax=ax, shrink=0.85, pad=0.10)
    cbar.set_label("ルール数", fontsize=9, color=C_TEXT_SUB)
...
def make_net_sales_multi_tags() -> None:
    ...
    ax.set_yticks(y)
    ax.set_yticklabels(grouped.index, fontsize=9.5, family="monospace")
    ax.invert_yaxis()
    ax.set_xlabel("紐づく XBRL タグ数", fontsize=10.5, color=C_TEXT_SUB)
...
def make_field_coverage() -> None:
    ...
    for i, (_, r) in enumerate(top.iterrows()):
        ax_l.text(r["coverage"] + 1, i, f"{r['coverage']:.1f}%",
                  va="center", fontsize=9.5, fontweight="bold", color=C_TEXT)
    ax_l.set_yticks(y)
    ax_l.set_yticklabels(top["field"], fontsize=8.5, family="monospace")
    ax_l.invert_yaxis()
    ax_l.set_xlabel("カバレッジ（%）", fontsize=10, color=C_TEXT_SUB)
    ax_l.set_xlim(0, 110)
    ax_l.set_title(f"高カバレッジ Top 15  ―  全銘柄で安定取得可能",
                   fontsize=11.5, fontweight="bold", color=C_TEXT, pad=10, loc="left")
    ...
    for i, (_, r) in enumerate(low_subset.iterrows()):
        ax_r.text(r["coverage"] + 1, i, f"{r['coverage']:.1f}%",
                  va="center", fontsize=9.5, fontweight="bold", color=C_TEXT)
    ax_r.set_yticks(y)
    ax_r.set_yticklabels(low_subset["field"], fontsize=8.5, family="monospace")
    ax_r.invert_yaxis()
    ax_r.set_xlabel("カバレッジ（%）", fontsize=10, color=C_TEXT_SUB)
...
def make_schema_principles() -> None:
    ...
    for i, ((num, title, desc, example), color) in enumerate(zip(principles, colors_box)):
        ...
        # タイトル
        ax.text(1.6, y_top - 0.20, title, fontsize=12.5,
                fontweight="bold", color=color, va="center")
        # 説明
        ax.text(1.6, y_top - 0.65, desc, fontsize=9.5,
