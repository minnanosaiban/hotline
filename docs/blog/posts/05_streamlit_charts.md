...
連載01〜04 では「コンセンサスデータ」「株価」「決算発表日時」を使って銘柄を分析してきました。本記事はおまけ回として、これらのデータをどこから取得し、どうやって Streamlit でブラウザに表示するか ― データのとり方とアプリファイルの連携方法を説明します。また、ちょっと役立ちそうな  **3つのチャートアプリ**  を紹介します。

| アプリ         | 用途                                  |
| ----------- | ----------------------------------- |
| **Chart 1** | GU/GDを分けりやすく！縦線入り5分足ローソクチャート        |
| **Chart 2** | 複数銘柄のトレンドと指標を比較！4列に並べたラインチャート       |
| **Chart 3** | 見てびっくり！決算発表後の値動きをパターンが分かりやすく並べたチャート |
