...
連載05 の [マルチファクタースコアボード](05_multifactor_scoreboard.md) まで、銘柄を「量」と「水準」で採点してきました。ここで、 「アクルーアル比率が低い銘柄（CF 裏付け強）は、高い銘柄を年率約 10% アウトパフォーム」というSloan の発見があります。本記事は **純利益と営業キャッシュフローの乖離（アクルーアル）** で、利益の "質" を見抜きます。

題材は連載03 で見た **ＥＮＥＯＳ の 2022 純利益 5,371 億円ピーク** ― この "特需" は本当にキャッシュになっていたのか、を検証します。
...
連載03 で構築した自前パイプラインから、有報 XBRL JSON 91 期分（EDINET 13 銘柄 × 7 期）を読み込み、アクルーアル比率を計算しました。

### ＥＮＥＯＳ ― 2022 ピーク利益はキャッシュで裏付けられていなかった

連載03 で示した「ＥＮＥＯＳ 2022 ピーク純利益 5,371 億円」を、純利益と営業 CF の両方で見直すと、根本的に違う景色が見えてきます。
