...
| 時間軸 | シグナル | ＥＮＥＯＳの状態 |
|---|---|---|
| **短期（連載15 CAR [−1, +1/+5]）** | [−1,+1]=+1.36% で類似群より上回る → [−1,+5]=−4.68% で反転（類似群−1.56% を下回る） | 初動は配当維持・市況回復シナリオを好感（在庫影響除き 4,744 億で「実質 4,400 億維持」を上回って着地）。5 日では高 OPM セグメント正常化懸念が後追いで反映 |
...
**[−1, +5] が確定し −4.68%（類似群平均 −1.56% を −3.12pp 下回る）**。初動 +1.36% が 5 日で反転した事実は、連載12 で観察した「高 OPM セグメントの正常化局面」と整合し、「短期サプライズは持続しなかった」ことを示します。[−1, +20] については引き続き継続観察が必要です。
...
- **ＥＮＥＯＳ 2026/3 期**：初動 [-1,+1]=**+1.36%（ポジショック）** だったが 5 日では **−4.68%（類似群 −1.56% を下回る）** に反転。在庫影響除き 4,744 億で「実質 4,400 億維持」を上回って着地という数字は初動好感されたが、高 OPM セグメント正常化懸念が後追いで反映。短期・中期・長期で見方が分かれる典型的な複層銘柄（連載01 4 基準試算・連載12 と整合）
