...
# 値動きの「共動の崩れ」で突発材料を検出する ― 市場が動かないのに、その銘柄だけ動いた日

![突発材料の異常検知](img/15_price_anomaly/00_thumbnail.png){width="1280"}

前回（3-5）、値動きの相関で「いつも一緒に動く仲間（クラスタ）」が見えました。ここで自然な問いが生まれます ―「**その仲間から、ある銘柄だけが突然はずれる日はあるのか？**」。
...
どの銘柄も「市場全体の波」と「業種の波」に乗っています。そこで **PCA（主成分分析）** で全銘柄に共通する波（上位 10 本で分散の 51%、うち PC1=36% が市場全体）を取り出し、各銘柄の動きを **その共通成分でどれだけ再現できるか** を見ます。
...
- 上段：セブン＆アイの 2024-08-19、**実際の動きは約 9σ** 跳ねたのに、共通成分（青線）はほぼ 0 ― 市場・業種では説明できない
...
異常度（残差 σ）の上位を、**その日の「個別超過リターン（銘柄 − 市場平均）」** で並べます。
...
## ＥＮＥＯＳ ― いつもの石油ピアから外れた日

本連載の中核 **ＥＮＥＯＳ** で試します。通常は石油ピア（出光・コスモ）と **相関 0.79** で連動しますが、ローリング相関は **0.47 まで急落** しました ― その日は **2025-03-28 の業績予想修正の前後**。
...
これで機械学習編（3-1〜3-6）が一区切りです。**予測（3-2 K-NN／3-4 RF）は構造的に難しい一方、発見（3-1 類似検索／3-3・3-5 クラスタリング／3-6 異常検知）は教師なしでよく効く** ― 「AI は予言の魔法ではなく、整理・発見・時間節約の道具」という本連載の結論を、値動きデータが最も鮮やかに示しました。
