...
連載01〜05 では、証券会社が無料で提供する銘柄情報サービスから取れる **集計済み数値** だけで業績軸（PEG / マルチファクター / リビジョン / サプライズ）と需給軸（信用需給）の立体評価を組み立てました。しかしそこには明確な限界があります。
...
これは連載01〜05 のスナップショットには映りません。同じ会社の **7 年間の構造的なピークアウト** は、無料で取れるデータの枠を超えた **有価証券報告書 XBRL** からしか取り出せません。
...
連載01〜05 は強力なフレームワークでしたが、本質的に **スナップショット分析** です。証券会社が無料で提供するデータは「直近時点の集計済み数値」を提供してくれますが、以下のような情報には届きません。
...
![既存サービスと自前 XBRL のデータ深度](img/06_xbrl/04_data_depth_comparison.png){width="950"}
...
![ＥＮＥＯＳ ピークアウト](img/06_xbrl/05_eneos_peakout.png){width="950"}
...
なお比較基準を **2019 年（純利益 3,223 億円・ROE 12.3%）= コロナ前の平常期** に置くと、2025 は −30% 程度で「ピーク比」より穏やかな比較になります。さらに 2025 の純利益は **のれん減損 ▲1,600 億・在庫影響 ▲1,500 億** という一時／構造要因を含むため、ENEOS自身は「実質営業利益 4,400 億円維持」と主張しています（4 基準試算は <a href="01_PEG_ROE銘柄分析.md">連載01</a> 参照）。
...
つまり連載05 で示した「業績NG × 踏み上げ罠 vs 構造を見抜いた仕込み」二項解釈は、**7 年スパンで見れば 2022 ピークの記憶 vs 業績の構造的低下** という対比軸として再解釈できます。アナリストは「構造的低下」を、個人投資家は「2022 の戻り or 一時要因の剥落後の本業実態 (4,400 億ライン)」を見ている、と読むこともできます。どちらが正しいかは有報 XBRL × 連載01 の 4 基準試算で前期比較するまで断定できません。

### 石油元売 3 社の 7 年比較 ― 純利益と ROE

ＥＮＥＯＳ 単独の構図を、3 社で比較すると違いが明確になります。

![石油元売3社 純利益・ROE 7年](img/06_xbrl/02_oil_3companies_ni_roe.png){width="950"}
...
![3 社 CF 3 種 7 年](img/06_xbrl/03_oil_3companies_cf.png){width="950"}
...
![3 社 売上・営業利益 7 年](img/06_xbrl/01_oil_3companies_revenue_oi.png){width="950"}
...
<div class="margin01">
<div class="card-bule">
<p class="small"><b>📝 XBRL でも取れないもの ― ENEOS 独自指標の壁</b></p>
<p class="small pad2">本記事の 7 年時系列は IFRS 標準勘定の <b>純利益・ROE</b> を取り出しています。一方、ENEOS が PR で打ち出す <b>「実質営業利益（在庫影響除き・非継続含む）4,400 億円」「一過性除き本業利益 4,250 億円・ROE 7.9%」</b> は会社独自の組替え定義であり、有報 XBRL の標準勘定には該当タグがありません。</p>
<p class="small pad2">これらは XBRL の要素データ（営業利益・のれん減損・在庫評価損益・非継続事業損益）を取得してから <b>自前で同じ計算を再現</b> する必要があります。連載08 のスキーマ設計で実装方針を、フェーズ 3（連載10〜）で 2024/3 期との <b>前期比較（本当に "維持" か）</b> を扱います。XBRL は万能ではなく、企業独自指標を <b>裏付ける／反証する "再構築の土台"</b> です。</p>
</div>
</div>
...
- 連載01〜05 で扱った証券会社が無料で提供するデータは **集計済みのスナップショット** で、過去 7 年の業績トレンド・セグメント情報・CF 詳細などには届かない
