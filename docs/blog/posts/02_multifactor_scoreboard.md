...
「自社・取引先・競合の銘柄を 7 ファクターで並べて、攻め型・守り型・バランス型を視覚比較したい」 ― そのために本記事のレーダーチャートを、**3 列カードグリッド**で複数銘柄を俯瞰できる Streamlit アプリにまとめました。**Python ファイル 1 つ・200 行台のコード** で、TOPIX 500 内パーセンタイル採点（相対評価）と銘柄カードグリッドが完成します。**「watch list だけ表示」「universe を変える」「ファクター定義をカスタマイズ」** といった改造も、コードを数行書き換えるだけ。

<small style="color: var(--md-link-color);"><i class="fa-solid fa-expand"></i> クリックで拡大できます</small>

![マルチファクター・スコアボード アプリ画面](img/02_multifactor/app.png){width="1200"}

> 🔗 [`github.com/minnanosaiban/blog/02_multifactor`](https://github.com/minnanosaiban/blog/tree/main/02_multifactor) ― `pip install -r requirements.txt` → `streamlit run app.py` で起動（初回は TOPIX 500 の価格メトリクス計算に 30 秒程度かかります）。
