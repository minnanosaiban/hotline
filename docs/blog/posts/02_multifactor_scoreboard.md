...
「自社・取引先・競合の銘柄を 7 ファクターで並べて、攻め型・守り型・バランス型を視覚比較したい」 ― 本記事のレーダーチャートを、**3 列カードグリッド**で複数銘柄を俯瞰できる Streamlit アプリにまとめました。**Python 1 ファイル・200 行台** で、TOPIX 500 内パーセンタイル採点（相対評価）と銘柄カードグリッドが完成。watch list 表示・universe 変更・ファクター定義のカスタマイズも、コードを数行書き換えるだけです。

<small style="color: var(--md-link-color);"><i class="fa-solid fa-expand"></i> クリックで拡大できます</small>

![マルチファクター・スコアボード アプリ画面](img/02_multifactor/app.png){width="1200"}

> 🔗 [`github.com/minnanosaiban/blog/02_multifactor`](https://github.com/minnanosaiban/blog/tree/main/02_multifactor) ― `pip install -r requirements.txt` → `streamlit run app.py` で起動（初回は TOPIX 500 の価格メトリクス計算に 30 秒程度）。
