...
ここまでで、銘柄分析の土台となる **データ取得の引き出し**がそろいました。

|      | テーマ               | 取得したデータ（取得元）                                                             |
| ---- | ----------------- | ------------------------------------------------------------------------ |
| 1 | 株価を取得する           | 日足・5分足 ― parquet に蓄積（yfinance）                                           |
| 2 | 株価以外を取得する         | 有報・決算短信の XBRL（EDINET・TDnet）/  決算発表日時（TDnet）/ 業績指標 EPS・ROE・予想修正率（証券会社アプリ） |
| 3 | XBRL を JSON に変換する | 統一 JSON ＝ 7 年の時系列業績指標（自前パーサで XBRL を変換）                                   |
