...
連載07-08 のパイプラインから 決算短信 actual 327 件と forecast 327 件を取得し、証券会社のコンセンサス EPS予想（1,457 銘柄）と (code) で結合。EPS の絶対値が 1 円以上で発散しない **211 銘柄** で三角検証が可能になりました。
...
2. **連載11 の三角検証**: 上方修正期待があるか？ → 全社 OK（特に住友商事 / 三菱商事）
...
- ＥＮＥＯＳ コンセンサス EPS（証券会社提供）: **142.81 円**
- コンセンサス − 実績 = **+78.6%**（アナリストの来期成長期待）

連載04 で観察された「ＥＮＥＯＳ EPS 予想超過率 +48.5%」は、**この +78.6% という来期成長期待の現れ**。同時に「経常利益変化率(予) −7.2%（来期減益予想）」も併存しており、一見「EPS は伸びるが経常は減益」と矛盾するように見えます。
...
- ＥＮＥＯＳ は決算短信 XBRL 未取得のため二角検証（実績 80 円 × コンセンサス 143 円 = +78.6% 来期期待）。EPS 来期 +78.6% と 経常変化(予) ▲7.2% の併存は、**のれん減損 / JX金属IPO 益 / 自社株買い** で構造的に説明可能（連載01 4 基準試算参照）。完全な三角検証には決算短信 XBRL の追加取得が必要
