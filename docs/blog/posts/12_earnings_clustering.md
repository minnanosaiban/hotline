...
# クラスタリングで「決算の型」を発見する ― 教師なし学習が分けた 3 プロファイル
...
連載 3-1 で設計した **10 次元の数値特徴量**（売上・営利・税引前・純利・包括の YoY、配当成長、営業利益率、純利益率、セグメント数、主力依存度）をそのまま使います。z-score で正規化し、**k-means** で「正解ラベルなしに似た決算をグループ化」します ― これが**教師なし学習**です。

<div class="keypoint" markdown="span">**クラスタ数 K** は、分離の良さを測る **シルエット係数** で決めます。</div>
...
- シルエットは **K=2〜3 が高く（0.30 前後）、K≧4 で 0.19 以下に低下**。決算は本質的に「少数の型」に分かれることを示します
- 本記事は **解釈しやすい K=3** を採用。2 つだと「高収益 vs その他」で終わり、4 つ以上は分離が弱まるため

## 型マップで「3 つの決算プロファイル」を観測

10 次元を PCA で 2 次元に落として可視化すると、**3 つの型がはっきり分かれます**。
...
連載 1-3 で見た「純利 **5,371 → 2,261 億の半減**（ピークアウト）」の、ちょうど **反転局面** を、k-means が**教師なしで「急回復型」と分類**した形です。「ピーク → 剥落 → 急回復」というシクリカルなサイクルが、銘柄名を伏せた数値だけから "型" として浮かび上がります。
