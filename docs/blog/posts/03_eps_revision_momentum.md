...
東証上場 2,586 銘柄（時価総額 100 億円以上を対象）について、業績予想修正率と株価指標を組み合わせて 4 象限分析を行いました。

### 全市場のリビジョン地合い

![市場別リビジョン](img/03_revision/01_market_revision.png){width="1200"}

**全市場の修正比率は 1.18 ＝ 中立** ですが、市場別に見ると緩やかな分化があります。

| 市場 | 上方 / 下方 | 修正比率 | 判定 |
|---|---|---|---|
| 東P（プライム） | 389 / 332 | **1.17** | 中立 |
| 東S（スタンダード） | 34 / 30 | **1.13** | 中立 |
| 東G（グロース） | 26 / 28 | **0.93** | 中立寄り弱気 |

3 市場ともに中立圏内で、大きな差は見られません。東P が若干リード（1.17）、東S も同程度（1.13）で前回ほどの優位はなく、東G のみ 0.93 とわずかに弱気寄りです。**全体として銘柄選定の質で勝負する局面** と判断できます。
...
|---|---|---|---|---|
| **コスモエネＨＤ** | **+1.11%** | +0.03% | 5.5 | 微上方修正、横ばい |
| **ＥＮＥＯＳ** | **−3.71%** | −0.57% | 9.4 | **下方修正かつ株価も軟化** |
| **出光興産** | **−3.48%** | −1.04% | 8.1 | **下方修正かつ株価も軟化** |

連載01・02 で観察された構図が、ここで **大きく変わる兆し** が読み取れます。

```
連載01 (PEG×ROE):    コスモ 理想ゾーン (-5.2%下落) / ENEOS バリュー候補 (+29.7%上昇)
連載02 (マルチファクター): コスモ Consensus 73 / Sentiment 21 / Momentum 39 → 乖離
連載03 (リビジョン):    コスモ +1.11%上方  / ENEOS -3.71%下方  / 出光 -3.48%下方
                       ─── 役者が逆転 ───
```

連載01・02 で **「GARP 圏外なのに +29.7% 上昇していた」** ＥＮＥＯＳ・出光が、直近で **下方修正** を受けています。株価も下落に転じています（−0.57% / −1.04%）。これは 4 象限マップでは **「底入れ待ち」** ― 下方修正と株価下落が揃った状態で、市場が悪材料を織り込み始めています。

**ＥＮＥＯＳ 関係者・株主にとっての示唆**:

- 連載02 時点で観察された Consensus 13（低）／ Sentiment 51（中庸）の組み合わせは、本記事のリビジョン −3.71% に直接つながっている
- 「ROE 8.0% でも +29.7% 上昇できた」のは、業績予想上方修正への先回り買いだった可能性が高い
- 現在は **「下方修正が株価に織り込まれるプロセス」** が進行中 ― 底入れ待ちゾーンに入りつつある。ただし下方修正の主因が **のれん減損（非現金）・在庫影響（油価連動）** であるため、本業実態とリビジョン値の方向感が分かれる局面（下記 callout 参照）
...
<p class="small pad2">本記事が使う証券会社が無料で提供している値 <b>▲3.71%</b> は <b>EPS コンセンサス基準</b> の修正率です。しかし同じ ＥＮＥＯＳ 2025/3 期業績予想修正でも、基準を変えると <b>営業利益ベース ▲94%</b>、<b>純利益ベース ▲2.27%</b>、<b>ENEOS 実質基準 +4.76%</b> と <b>▲94% 〜 +4.76% のレンジに分散</b> します（連載01 編集部試算）。</p>
...
### リビジョン × 株価モメンタム ― 出遅れ買い候補 90 銘柄

![リビジョン×モメンタム散布図](img/03_revision/04_revision_vs_momentum.png){width="1200"}

時価総額 100 億円以上・ROE 5% 以上にフィルタした 1,965 銘柄について、4 象限分類すると（散布図の表示範囲内で）**出遅れ買い候補が 90 銘柄、逆行注意が 28 銘柄**。石油元売 3 社はいずれも中央付近に位置しており、ＥＮＥＯＳ・出光 は左下の「底入れ待ち」ゾーンに移行しつつあります。
...
### リビジョン × バリュエーション ― 修正済み割安 66 銘柄

![リビジョン×バリュエーション散布図](img/03_revision/05_revision_vs_valuation.png){width="1200"}

修正率 ≥ 3% かつ PER（予）≤ 15 の **「修正済み割安候補」は 66 銘柄**。業績改善が割安価格で買える状態です。バフェット流の "クオリティ × バリュー × モメンタム" の交差点で、機関投資家のファンドマネージャーが必ずチェックする領域です。

| 銘柄 | 修正率 | PER予 | ROE | 値上り率 |
|---|---|---|---|---|
| ＫｅｅＰｅｒ技（6036） | +39.1% | 7.7 | 30.1% | +1.1% |
| 三洋貿易（3176） | +29.2% | 9.2 | 9.3% | +1.6% |
| アカツキ（3932） | +20.3% | 8.0 | 13.1% | -0.9% |
| 東洋電（6505） | +20.0% | 7.7 | 8.0% | +1.1% |
| 第一生命（8750） | +15.1% | 12.8 | 11.1% | -1.1% |
| 神島化（4026） | +15.0% | 10.6 | 11.6% | +4.5% |

### リビジョン強度ランキング

![リビジョン強度ランキング](img/03_revision/03_revision_strength.png){width="1200"}
...
- 修正比率（上方 / 下方）で市場全体の地合いを判定: 今回は **全市場 1.18（中立）/ 東P 1.17（中立）/ 東S 1.13（中立）/ 東G 0.93（弱気寄り）**
- 4 象限マップで **出遅れ買い候補 90 銘柄 / 逆行注意 28 銘柄 / 修正済み割安 66 銘柄** を抽出
- 連載01・02 で +29.7% 上昇していた **ＥＮＥＯＳ / 出光が直近 −3.71% / −3.48% の下方修正、株価も −0.57% / −1.04% と下落に転じ「底入れ待ち」ゾーンへ**。一方コスモエネＨＤ は +1.11% の小幅上方修正で横ばい ― 連載01〜02 と役者が逆転している可能性
- ＥＮＥＯＳ の ▲3.71% は **EPS コンセンサス基準**。基準次第で ▲94% 〜 +4.76% に分散する（連載01 4 基準試算）。リビジョン値の "中身" を見ないと本業悪化と一時／構造要因を取り違えるリスク
