...
「上方修正された銘柄に注目したいが、すでに株価が反応していたら出遅れだ」 ― そのズレを 4 象限マップで一目で判定できるのが **Streamlit + Plotly**。**Python ファイル 1 つ・100 行台のコード** で、ホバー表示・ズーム・パンが揃った散布図ダッシュボードが完成します。**「自分の watch list だけ表示」「業種ごとに絞り込む」「経常変化率も重ねる」** といったカスタマイズも、コードを数行書き換えるだけ。
