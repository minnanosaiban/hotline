...
ただし決算短信 XBRL は **時系列が 2 年分** しかないので、本記事は上の 5 年版スコアではなく、**前期比（YoY）成長率の大小**で「加速／減速」を見ます。5〜7 年のトレンドは過去まで遡れる有報 XBRL で取れますが、有報のセグメント読み取りは整備中（筆者の今後の課題）。今回の対象は、短信でセグメントが取れた 233 銘柄 × 787 セグメントです。
...
まずは身近な 1 社、ソニーG から。マルチファクター分析では総合スコア 44.1 と平均的な評価でしたが、決算短信 XBRL でセグメントに分解すると、別の顔が見えてきます。
...
- **音楽事業は利益率 21.4% でゲーム（10.1%）の 2 倍以上** ― 売上はゲームの半分ほどだが、音楽パブリッシング・原盤権という「資産をあまり持たずに稼ぎ続ける」型の高収益
- 総合スコア 44 という平均的な評価は、この音楽＋画像センサーの高利益率を企業合算で平均化し、過小評価していた可能性
- セグメント別に見ると **ソニーG は単なるゲーム会社でもエレキ会社でもない** ― 事業ポートフォリオの強みが浮かぶ

### セグメント前期比成長率（加速 / 減速 Top10）

次に、視野を全 233 銘柄 × 787 セグメントへ広げ、前期比成長率（売上 100 億円以上）の大きい順・小さい順に並べます。
...
- 加速トップは大東建託 不動産開発 +185.5%（1,471 億）。注目は **丸紅 次世代事業 +127%** と **双日 エネ・ヘルスケア +78.9%** ― 利益の質・予想の信頼性が確認された商社が、セグメントでも新規事業で伸びている
...
最後に、主要銘柄を 2 年分のセグメント別売上で並べます。ここで分かるのは、**セグメント開示は会社ごとにバラつき、前期比まで追える会社は限られる**という現実です。
