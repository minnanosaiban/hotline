...
その音楽事業は売上規模こそゲームの 46% 程度ですが、利益率はゲーム（10.1%）の 2 倍以上。Spotify / Apple Music が世界を席巻するストリーミング時代でも、音楽パブリッシング・原盤権という **「軽資産で永続キャッシュフロー」** ビジネスの収益性が際立ちます。
...
加速 Top1 は **大東建託（1878）の "不動産開発事業" +185.5%**（売上 1,471 億）。ストーリー上で注目すべきは **丸紅の "次世代事業開発" +127%**（売上 1,824 億）と **双日の "エネルギー・ヘルスケア" +78.9%**（売上 3,485 億）。利益の質（アクルーアル）が健全で予想の信頼性（予想検証）も確認された総合商社が、セグメントレベルでも **新規事業に成功している** 構図が確認できます。

逆に **丸紅は同時に "金融・リース・不動産" が −54.7% で減速 Worst1**。これは **事業転換期** にある典型的なサイン。「総合商社のうち丸紅は次世代事業 +127% × 既存金融事業 −55% の二極化」が、本記事のセグメント分析でしか見えない結論です。

最も衝撃的なのは **セブン&アイ（3382）の本業 "スーパーストア事業" が −47.4%**（売上 6,876 億）。これは大規模事業再編（イトーヨーカ堂分離等）の影響と推測されますが、企業合算では見えない構造変化が、セグメントで一発で可視化されます。
...
後編（[2-6 コングロマリット](08b_segment_core_stocks.md)）では、この発掘ツールを連載の中核銘柄に接続し、**総合商社（丸紅・双日）の事業転換** と **ＥＮＥＯＳ のピークアウト内訳** を深掘りします。アクルーアル（利益の質）・予想検証（予想の信頼性）・セグメントの 3 分析が同じ方向を指すかを確認します。
