...
# LightGBM で「次の 5 分」を当てられるか試す ― 111 万バーで検証
...
<p class="fig-meta">データ出典: 番外編A と同じ 5 分足・流動性上位 300 銘柄（2025-11-21〜2026-05-29）。学習 111 万バー（〜2026-03-31）→ 検証 51 万バー（2026-04-01〜、境界 1 日はパージ）。実装は `blog21_scalping_ml.py`</p>
...
- ところが、モデルの確信度で 51 万バーを 10 グループに割ると、次バーの平均リターンは **−0.66bps → +0.22bps へきれいに単調**
...
- ただし確信度十分位は **−0.66 → +0.22bps と単調** ― 並べ替え（構造の発見）はできている
