...
「この決算、どこかで見た形だな」― そんな直感を、機械にやらせてみます。決算の数字から **10 個の特徴量** を取り出し、**コサイン類似度**（数字の並びの "似ている度"）で「似た決算」を自動で探します。業種コードを一切使っていないのに、**商社・電機といった "業界の型" が数字だけから自然に立ち上がる** ― そこが本記事の見どころです。（見つけた類似群を "物差し" にして、その銘柄だけの異変＝個別ショックを拾う話は、次回 3-2 へ。）

<p class="fig-meta">データ出典: 自前パイプラインの `data/statements/*_2026-03-31_FY.json`（kind="actual" 232 銘柄）、`data/news/tdnet/2026-04~05-*.csv`（決算短信 3,549 件）。実装は `scripts/blog15_similarity.py`（特徴量抽出 + 類似度）と `scripts/blog15_generate_images.py`。実 embedding API は呼んでいません（数値特徴量のみで完結）</p>
...
ここまでの記事は「**個別銘柄の決算を評価する**」ものでした。本記事は逆に、**「ある決算に最も似た過去の決算」を機械的に引き出す** ― 検索エンジンのような使い方です。
...
- **embedding API は不要、手元の計算だけで完結** ― 1,000 銘柄の Top-15 が約 0.1 秒・コスト 0 円
- 対象は、決算短信 JSON のうち特徴量が 7 個以上そろう **232 銘柄**
...
| 4 | 6503 | 三菱電機 | 0.902 | +6.8% | +25.8% | +10.0% |
...
数値特徴量と LLM embedding（文章を AI が数値ベクトルに変換したもの）は、コスト・速度・分かりやすさ・捉えられる対象が違い、得意分野が分かれます。
...
**おすすめは両者の合わせ技です** ― まず数値特徴量で高速に Top-50 まで絞り（コスト 0）、次に LLM の要約・embedding で Top-10 に精密化する（コスト数円）。こうすれば、**「数字は似ていても経営の中身が違う」銘柄を 2 段目で外せます**。次回は、この類似検索を未来予測へ発展させます。

---

## まとめ

- **検索エンジン的アプローチ** ― 10 個の数値特徴量 + コサイン類似度で「最も似た決算」を発見（embedding API 不要・1,000 銘柄 0.1 秒・コスト 0 円）。LLM embedding と併用すれば「数字は似ても中身が違う」銘柄も外せる
