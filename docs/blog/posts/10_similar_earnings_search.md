...
<p class="fig-meta">データ出典: 自前パイプラインの `data/statements/*_2026-03-31_FY.json`（kind="actual" 287 銘柄）、`data/news/tdnet/2026-04~05-*.csv`（決算短信 3,549 件）。実装は `scripts/blog15_similarity.py`（特徴量抽出 + 類似度）と `scripts/blog15_generate_images.py`。実 embedding API は呼んでいません（数値特徴量のみで完結）</p>
...
- 対象は、決算短信 JSON のうち特徴量が 7 個以上そろう **287 銘柄**
