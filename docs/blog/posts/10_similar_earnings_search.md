...
連載03 で構築した **構造化 JSON** から、本記事では **10 次元の数値特徴量ベクトル** を抽出し、**コサイン類似度** で「似た決算」を発見します。embedding API も LLM も呼ばず **ローカル計算 0 円で完結** する実装です（※ JSON を LLM に渡して 1 文要約や embedding を作る手もあり、Haiku 4.5 でも 1,000 銘柄 約 400 円と安価ですが、本記事は 0 円の数値特徴量で進めます）。

ただし「類似決算検索」の真価は **発見した類似群の CAR と、クエリ銘柄自身の CAR を比較すること** にあります。本記事では連載09 で構築した CAR 計算を 2026/3 期 announcement にも拡張し、**丸紅（8002）の類似 Top-15 が平均 CAR +2.39%・勝率 60% だったのに対し、丸紅自身は -9.39%** という強い乖離を発見します。これは「数字パターン上は同業並みの好決算なのに、市場は個別要因で売った」という **数値類似だけでは捉えきれない市場の個別反応** です。
...
連載01〜09 はすべて「**個別銘柄の決算データを評価**」する記事でした。連載10 は「**ある決算に最も似ている過去の決算**」を機械的に発見します。

| 連載 | 視点 | 比較対象 |
|---|---|---|
| 02-05 | 証券会社の無料データでスコア化 | 全銘柄ランキング |
| 06-08 | XBRL 数値分析 | 全銘柄統計 |
| 09 | CAR 集計 | 全 8,049 イベント |
| **10 類似決算検索** | **特徴量ベクトル × cosine** | **「最も似た決算」Top-K** |

連載01〜09 までは「**この銘柄の決算を理解する**」記事。連載10 は「**この決算に最も似ている過去の決算を引き出す**」記事 ― 検索エンジン的アプローチです。
...
本記事で扱わないこと:
  ...
  ・類似決算の CAR 分布を「予測」として使う backtest（連載11 で扱う）
```
...
- **連載08 で見た事業ポートフォリオ転換**（「次世代事業 +127% × 金融・リース・不動産 -54.7% の二極化」）が決算説明会で詳細開示され、市場が **「次世代事業の利益化までの時間軸」を再評価** した可能性
...
3 銘柄に共通するのは **「数値特徴量パターンと市場反応が乖離している」** こと。連載 narrative にとって重要なのは、**「類似群がマイナス、ＥＮＥＯＳ がプラス」というポジティブ乖離が、連載08 で観察した「主力 2 事業の OPM ピークアウト」と一見矛盾する** ことです。

この一見の矛盾は、時間軸を分けることで解消できます:

| 時間軸 | シグナル | ＥＮＥＯＳの状態 |
|---|---|---|
| **短期（連載10 CAR [−1, +1/+5]）** | [−1,+1]=+1.36% で類似群より上回る → [−1,+5]=−4.68% で反転（類似群−1.56% を下回る） | 初動は配当維持・市況回復シナリオを好感（在庫影響除き 4,744 億で「実質 4,400 億維持」を上回って着地）。5 日では高 OPM セグメント正常化懸念が後追いで反映 |
| **中期（連載08 セグメント）** | 機能材 OPM −1.82pt / 開発 OPM −12.54pt | 高 OPM 事業の正常化局面（「構造的ピークアウト」か「サイクル正常化」かは 2 年比較では断定できず、継続観察必要） |
...
ＥＮＥＯＳ は **「短期は類似群を上回り、中期〜長期は『数値上の縮小』と『ENEOS 主張の実質維持』が並走する」** 複層銘柄です。数値類似度 + CAR だけで判断すれば「ポジショック銘柄」に見えますが、連載08 のセグメント情報（高 OPM 事業の正常化局面）と連載06 の 3 年累積 CF 回収（114%）、連載04 4 基準試算（▲94% 〜 +4.76%）を組み合わせると、**短期サプライズの裏で複層的な構造変化が同時進行している** より複雑な実像が浮かびます。「構造的劣化」と読むか「サイクル正常化と本業実質維持」と読むかは、採用する基準次第です。

**[−1, +5] が確定し −4.68%（類似群平均 −1.56% を −3.12pp 下回る）**。初動 +1.36% が 5 日で反転した事実は、連載08 で観察した「高 OPM セグメントの正常化局面」と整合し、「短期サプライズは持続しなかった」ことを示します。[−1, +20] については引き続き継続観察が必要です。
...
このグラフが連載10 の **「予測ツールとしての限界と発見ツールとしての価値」** を最も端的に表現しています。
...
LLM 要約・embedding と組み合わせれば、**「数字は似ているが経営の物語が違う」銘柄を 2 次フィルタで排除** できます。連載11 ではこの類似検索を未来予測へ発展させます。
...
連載03 スキーマの **`performance.change_pct`** ブロックがそのまま使えるため、抽出ロジックは 1 関数で完結します。`segment_count` と `max_seg_share` は連載08 で実装したセグメント情報の再利用です。
...
連載09 で構築した `events.parquet` は 2025-08 まで。2026/3 期 announcement の CAR は別途計算：
...
連載09 で書いた CAR 計算ロジックの **再利用** です。短期ウィンドウ（[-1,+1] / [-1,+5]）のみ計算 ―（[-1,+20] は 2026-05-21 時点で発表後 20 営業日経過しておらず未完了）。
...
`data/news/tdnet/2026-*.csv` は 5 桁、`data/statements/` 内 JSON は 4 桁。連載09 までは kessan/ ディレクトリのみ使っていたためこの問題は表面化していませんでした。連載10 で初めて 2026 年データに踏み込み、5 桁正規化ロジックを追加。
...
本記事執筆時点（2026-05-21）では 2026/3 期 announcement から **まだ 20 営業日経過していない** 銘柄が大半。連載09 で扱った [-1,+20] と直接比較できないため、本記事は **[-1,+5] までで議論を統一**。2026 年 6 月以降に再計算すれば [-1,+20] も追加可能。

---

## まとめ

- 連載10 は **「最も似た決算 Top-K を発見」する検索エンジン的アプローチ**。10 次元の数値特徴量 + cosine 類似度で実現（embedding API 不要）
...
- **ＥＮＥＯＳ 2026/3 期**：初動 [-1,+1]=**+1.36%（ポジショック）** だったが 5 日では **−4.68%（類似群 −1.56% を下回る）** に反転。在庫影響除き 4,744 億で「実質 4,400 億維持」を上回って着地という数字は初動好感されたが、高 OPM セグメント正常化懸念が後追いで反映。短期・中期・長期で見方が分かれる典型的な複層銘柄（連載04 4 基準試算・連載08 と整合）

次回連載11 は **過去類似決算の値動き分布から未来予測を構築** する記事に進みます。本記事の Top-15 類似決算群の CAR を **「予測分布」** として扱い、現在銘柄の今後の値動きをモンテカルロ的に推定する枠組みを作ります。連載10 で見えた「類似群と自身の乖離」の検出も組み込み、**「典型パターン or 個別ショック」を事前判定** できるダッシュボードに発展させます。
