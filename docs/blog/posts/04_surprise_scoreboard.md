...
本記事では修正率に **EPS 予想超過率・経常利益変化率（予）** を加えた **3 シグナルを合成**してサプライズスコアを作り、業績モメンタムが本物の銘柄を発掘します。連載01〜03 で追ってきた ＥＮＥＯＳ / 出光興産 / コスモエネＨＤ の状況も、3 シグナルで再評価します。注目すべきは **ＥＮＥＯＳ の経常利益変化率(予) −0.6%** ― 連載03 の下方修正に加え、来期もわずかながら減益予想となっています。
...
東証上場 3,970 銘柄について 6 指標を読み込み、サプライズスコアを算出して分析しました。フィルタは時価総額 100 億円以上・ROE 5% 以上・修正率 ≥ 0%（783 銘柄）が基本です。
...
| 6 | 三和油化工業（4125） | 88.3 | +12.7% | +21.9% | +12.7% | 8.4% |
| 7 | ＩＢＪ（6071） | 88.1 | +7.5% | +30.9% | +7.5% | 22.4% |

Top1 の **日清紡ＨＤ** は修正率 +16.0% / EPS 超過率 +38.5% / 経常変化率(予) +28.9% と、3 シグナルすべてが市場上位に並ぶ **業績モメンタム全方位タイプ**です。
...
- 経常変化率(予) ― データ欠損（本記事では評価不能）

連載01 の "GARP 理想ゾーン"、連載02 の Value 85 / Consensus 73 と、一貫して高評価です。経常データがないため 3 シグナル全揃いとはいえませんが、揃っている 2 シグナルは**業績モメンタムの方向と一致**しています。
...
EPS 超過率が +44% と高いのに、経常変化率(予) は **マイナス**。これはどういう構造か。**「EPS は今期実績比で大きく増えるが、経常利益は前期比で減る」** という、一見矛盾する組み合わせです。理由として考えられるのは:
...
修正率（X）× 経常利益変化率(予)（Y）の散布図で 4 象限を見ると、**右上ゾーン（上方修正 × 来期成長予想）が 32 銘柄** あります。
...
![合成スコア Top10 と単一 Top10 の重なり](img/04_surprise/04_signal_overlap.png){width="1200"}
...
![3 シグナル分布](img/04_surprise/05_signal_distribution.png){width="1200"}
...
サプライズスコア ≥ 90    → 上位 10%（本命級。本記事では 4 銘柄）
右上ゾーン: 修正率 ≥ +3% かつ 経常変化率(予) > 0
```

サプライズスコア 75 以上が 61 銘柄、90 以上が 4 銘柄。**4 銘柄まで絞れる強力な指標** であることが分かります。
...
- 4 象限散布図では **右上「上方修正 × 来期成長予想」32 銘柄** が業績モメンタムの本命ゾーン。修正の方向と来期見通しは強く相関するため、左上（回復期待）と右下（ピークアウト）はほぼ空白
- **石油元売 3 社の構図**: コスモエネＨＤ は修正率・EPS超過がプラス（経常データ欠損）/ ＥＮＥＯＳ は修正率 -3.7% × EPS 超過 +43.8% × 経常変化率(予) **-0.6%** で **方向がバラつく状態** ＝ エントリー保留が定石
...
| 02 (マルチファクター) | Cons 73 / Sen 21 → 乖離 | Cons 13 / Mom 60 | 中庸型 |
| 03 (リビジョン) | +1.11% 横ばい | -3.71% 底入れ待ち | -3.48% 底入れ待ち |
| **04 (サプライズ)** | **修正率・EPS超過プラス（経常欠損）** | **方向バラつき（経常 −0.6%）** | **方向バラつき** |

連載01〜02 で短期的に大きく上昇していた **ＥＮＥＯＳ / 出光 が直近で 3 シグナルがバラつく状態**、一方 **コスモエネＨＤ は連載01〜04 を通じて一貫してポジティブなシグナル**（経常データ欠損を除く）を示す結果となりました。
