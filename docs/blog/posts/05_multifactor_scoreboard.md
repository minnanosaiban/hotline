...
# マルチファクターで「全方位の優等生」を見つける ― 7 ファクター・スコアボード
...
「コスモはファンダ最良なのに株価は下落」という謎は、**ファンダ（Value 92 + Consensus 68）と需給（Sentiment 22 + Momentum 35）のスコアが乖離している** ことで定量的に説明できます。
...
これがマルチファクター採点の価値です。**ファンダだけ・需給だけでは見えない「乖離」が一目で分かる**。
