...
これがマルチファクター採点の価値です。**ファンダだけ・需給だけでは見えない「乖離」が一目で分かる**。次回からは決算短信そのものを扱うフェーズ3 に進み、この「質」をさらに深く検証していきます。
...
次回からは **フェーズ3（XBRL 活用編）**。証券会社の無料データから一歩進み、有報・決算短信の XBRL を直接分析します。第一弾の **アクルーアル分析** では、利益の「質」を営業 CF と純利益の乖離から見抜きます。
...
> [<span style="color: var(--md-link-color);">github.com/minnanosaiban/blog/05_multifactor</span>](https://github.com/minnanosaiban/blog/tree/main/05_multifactor)
