...
![決算短信JSONスキーマ](img/08_schema/00_thumbnail.png){width="1280"}
...
連載07 で示したように、証券会社が無料で提供するデータを超えたい個人投資家にとって XBRL は強力ですが、**それを毎回直接扱うのは現実的ではない**。「決算データ分析」と「XBRL タクソノミ理解」を切り離す層が必要です。
...
![セクション別ルール分布](img/08_schema/01_section_distribution.png){width="1200"}
...
![会計基準 × カテゴリ ヒートマップ](img/08_schema/02_standard_category_heatmap.png){width="1200"}
...
1. **performance は IFRS / JP の両方を厚く対応**（104 + 82）。連載06 でＥＮＥＯＳ（IFRS 採用）の業績を無料データと同等以上に取れたのは、この厚みのおかげ
...
![net_sales 系の 1 対 N 構造](img/08_schema/03_net_sales_multi_tags.png){width="1200"}
...
![実データのフィールドカバレッジ](img/08_schema/04_field_coverage.png){width="1200"}
...
| **企業独自指標の再構築** | **困難（要素を都度抽出）** | **スキーマ拡張で対応可能** |
...
<div class="margin01">
<div class="card-bule">
<p class="small"><b>📝 スキーマの真価: ENEOS 独自指標を要素から再構築する</b></p>
<p class="small pad2">連載06 で触れたとおり、ENEOS が PR で打ち出す <b>「実質営業利益（在庫影響除き・非継続含む）4,400 億円」「一過性除き本業利益 4,250 億円・ROE 7.9%」</b> は会社独自の組替え定義であり、XBRL の標準勘定タグには存在しません。</p>
<p class="small pad2">しかし本連載のスキーマで取得した要素データ（<code>operating_income</code> / 在庫評価損益 / のれん減損 / 非継続事業損益）を組み合わせれば、<b>企業独自の主張数値を XBRL レベルで自前計算できます</b>。これは「1 json_path : N xbrl_tag」設計の <b>最終的な価値</b> ― 公式 XBRL の標準項目だけでなく、企業独自指標も検証可能になります。</p>
<p class="small pad2">具体的な計算式と前期比較（本当に "維持" か）はフェーズ 3 で扱います。連載01 の <b>4 基準試算（▲94% 〜 +4.76%）</b> と、本連載のスキーマが連結することで、<b>読者が ENEOS の主張を自前で検証できる</b> 環境が完成します。</p>
</div>
</div>
...
- 連載06 で示したＥＮＥＯＳ 7 期データが、無料で取れるデータを超える価値を持つのは、この設計の上に成立している
- **企業独自指標（ENEOS の「実質営業利益 4,400 億円」等）も、XBRL 要素データを組み合わせれば自前計算可能**。連載01 の 4 基準試算とスキーマが連結し、フェーズ 3 で本格的な検証を行う

これでフェーズ 2（XBRL 基礎）の 3 本（連載06・07・08）が完成しました。次回からは **フェーズ 3: XBRL 活用分析** に入り、独自スキーマで蓄積した JSON を実際に使って **進捗率 Z-score 早期警報・アクルーアル分析・三角検証** などの本格的な銘柄分析を行います。連載01〜05 のフェーズ 1（証券会社が無料で提供するデータ分析）と連載06〜08 のフェーズ 2（XBRL 基盤構築）で築いた土台の上に、機関投資家レベルの定量分析が乗っていきます。
