...
「証券会社アプリで完結」していた個人投資家分析が、この連載を通じて **AI と機械学習を組み合わせた本格クオンツ並みのワークフロー** に発展しました。それでも年間数千円〜数万円。
...
- ネガショック Top: 5906 エンケイ（-28pp）/ 9629 PCA（-23pp）/ 6469 放電精密（-21pp）など、**資料熟読で見える警戒材料** で売られた銘柄
- 主要銘柄の最終結論:
...
ここまでで、データ取得から機械学習までの一連のワークフローが形になりました。証券会社アプリから始まり、XBRL 取得・パース・スキーマ設計を経て、最終的に **類似検索 × K-NN × CAR の三位一体ワークフロー** が個人投資家の手元で年間数千円〜数万円で運用可能になりました。**機械学習分析編には、今後も新しい手法を試して追加していく予定です。**
