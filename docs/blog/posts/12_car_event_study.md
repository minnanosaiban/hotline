...
![CARイベントスタディ](img/12_car_event_study/00_thumbnail.png){width="1280"}
...
| 連載 | 視点 | 株価との関係 |
|---|---|---|
| 01-05 | 証券会社の無料指標（PEG/ROE/修正率/サプライズ/信用） | 発表前の銘柄選定 |
...
本記事で扱わないこと:
  ...
  ・PEAD のロング/ショート戦略 backtest（連載13 以降）
```
...
![全イベント CAR 分布](img/12_car_event_study/01_car_distribution.png){width="1200"}
...
![ウィンドウ別サマリ](img/12_car_event_study/02_window_summary.png){width="1200"}
...
![短期 vs 長期 CAR 散布図](img/12_car_event_study/05_short_vs_long.png){width="1200"}
...
![narrative 5社サマリ](img/12_car_event_study/03_narrative5_summary.png){width="1200"}
...
- 丸紅は TOPIX 超過 +1.34%、セクター超過 +1.95% ― 業界を差し引いても **個別で +2% 弱の追加リターン**。連載08「アクルーアル健全」連載10「三角検証通過」連載11「次世代事業+127%」の評価が市場でも裏付けられた
...
![narrative 5社の時系列](img/12_car_event_study/04_narrative5_timeline.png){width="1200"}
...
直近通期（2025-05-02）は **TOPIX 超過 +8.94%、セクター超過も +8.76%**。「業界全体ではなく個別決算が効いた」と明確に切り分けられます。連載11 で見た「次世代事業 +127%」が市場で評価されたタイミングです。2024-08-01 の -9.74% は 8 月暴落の余波が混じった可能性が高い（同日に S&P/NEXT FUNDS も大きく崩れた）。
...
初動と長期がズレるイベントが目立ちます（2024-05-01 は [-1,+1]+7.84% でも [-1,+20]+0.38% でドリフトが消失）。連載 narrative では健全評価でしたが、**市場の反応は平均的で、丸紅ほどクリアな上昇トレンドにはなっていない** ― これは連載11 で双日について「商社2社のうち丸紅は narrative 圧勝、双日は事業転換シグナルあるが市場説明力は弱い」と評価したことと整合します。

### 連載01〜12 narrative との対応マップ

| 連載 | 評価 | 主要銘柄 | 連載12 CAR での答え合わせ |
|---|---|---|---|
...
| 11 三角検証 | 丸紅 C +1.8% | 8002 | 連載11 評価と二重で市場が肯定 |
...
**ＥＮＥＯＳ の CAR パターンは "上昇 4 連続 → 業績予想修正発表 (2025-03-28) で急落"**。短期の市場反応としては連載01-12 の bearish シグナル群と整合する動きですが、**下落の主因が のれん減損（非現金）+ 在庫影響（油価連動）+ JX金属 IPO 等の構造要因** であることを踏まえると、「**利益の質が完全に劣化した証拠**」とまで断定するのは早計です。連載09 で見た 3 年累積 CF/純利 = 114% や、ENEOS 自身の「実質営業利益 4,400 億円維持」主張（[連載04](04_garp_peg_roe.md) 4 基準試算参照）と併せて、複層的に解釈する必要があります。2025-05 以降の events や 2026/3 期 通期決算（連結営業利益 4,666 億の急回復、公表ベース +25.5% / 継続事業ベース +339.8%、在庫影響除き 4,744 億で「実質 4,400 億維持」主張を上回って着地、[連載11](11_segment_analysis.md) 参照）の CAR は本記事の集計範囲外で、継続観察が必要です。丸紅の narrative については、直近通期 CAR +8.94% で連載10-12 の「健全 + 次世代事業成長」評価と明確に整合しています。
...
- 連載01〜12 が **決算データの中身** だったのに対し、連載12 は **市場での答え合わせ**。連載03 で構築した自前パイプラインで **8,049 決算イベントの CAR を実証**
...
- **ＥＮＥＯＳの CAR パターン**：2024-05〜2025-02 で 4 連続 +5% 超 → **2025-03-28 の業績予想修正発表で [-1,+20] = -13.89%（セクター超過 -2.42%）に急転**。短期市場反応としては連載01-12 の bearish シグナル群と整合するが、下方修正の主因が **のれん減損（非現金）・在庫影響（油価連動）・JX金属 IPO** という構造／会計要因のため、「利益の質劣化の確証」と断定するには早計。連載08 の CF 累積 114% や ENEOS の「実質営業利益 4,400 億維持」主張、2026/3 期 通期決算 +542% 急回復（連載11）も併せて多面的に評価が必要
- **丸紅も narrative 整合**：直近通期（2025-05-02）で TOPIX 超過 +8.94%、セクター超過 +8.76%。連載08「健全」連載11「次世代+127%」が市場で評価された
- 1306.T / 1629.T の **連続株式分割を ratio 再スケーリングで補正**：yfinance auto_adjust が漏らした分割を chg_pct 異常値検出で自動修正

これでフェーズ 3（XBRL 活用分析）の最終本が完成しました。次回連載13 は **フェーズ 4（AI 統合）** に入り、**LLM による決算短信要約** に進みます。連載03 のスキーマと連載12 の CAR を組み合わせ、「**LLM 要約 × CAR 反応**」で要約品質を定量評価する枠組みを作ります。
