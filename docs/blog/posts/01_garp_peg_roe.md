...
PEG（Price Earnings to Growth ratio） は **PER を「成長率に対する割安度」に変換した指標** です。
...
ROE （Return On Equity）は **自己資本をどれだけ効率的に利益に変えているか** を示す指標です。
...
GARP（Growth At a Reasonable Price） は **PEG が低く（割安）、かつ ROE が高い（高品質）** 銘柄を選ぶ戦略です。
...
緑が **GARP 理想ゾーン**（PEG ≤ 1.0 かつ ROE ≥ 10%）です。理想ゾーンに入っている主要銘柄は **三井住友だけ**。**割安 × 高 ROE の両立は、大型株ではいかに稀か** ― チャートはそれを一目で示しています。

- **背景のグレーの小ドット** ― TOPIX 500 全銘柄（PEG ≤ 3.0 / ROE 範囲内のみ表示）
- **水色の中ドット** ― **TOPIX 大型 100 銘柄**（JPX の規模区分 *Core30 + Large70* で構成
...
数字だけ見ればコスモが圧倒的に魅力的に映ります。しかし直近の株価動向（**直近 6 ヶ月**）を見ると、印象が逆転します。
...
<p class="small pad2">ＥＮＥＯＳ は 2025 年 3 月 28 日に 2025/3 期営業利益を 11 月公表比 ▲3,950 億円下方修正しました。主因は <b>のれん減損 ▲1,600 億円</b>（2017 年 JX HD・東燃ゼネラル石油の経営統合に伴うのれんを、金利上昇等を受けて減損）と <b>在庫影響 ▲1,500 億円</b>。同時期に <b>JX金属 IPO</b>（57.6% 売却）も進行し、当期利益を +1,300 億円押し上げました。</p>
...
<p class="small pad2">同じ ＥＮＥＯＳ 2025/3 期業績予想修正でも、何を分子に置くかで「修正率」は大きく変わります。連載03 の証券会社が無料で提供している値 ▲3.74% と、公式 PDF から計算した 4 基準を並べると：</p>
<table class="table-set" style="font-size: 0.85em; margin: 0.5em 0;">
<thead><tr><th>基準</th><th>修正率</th><th>解釈</th></tr></thead>
<tbody>
<tr><td>証券会社が無料で提供する EPS予想</td><td><b>▲3.74%</b></td><td>コンセンサス基準（連載03）</td></tr>
...
<p class="small pad2"><b>同じ「修正」でも ▲94% から +4.76% まで分散</b>。連載03 のリビジョン値・連載04 のサプライズ・連載05 の需給判断を読むときは、この <b>4 基準のどれを採用するか</b> が投資判断の分かれ目になります。これは公式説明にも他のクオンツ記事にもない、本連載独自の試算です。</p>
</div>
</div>
...
次は日経 225 主要銘柄に視野を広げます。GARP スコア（ROE ÷ PEG を本表内で 0〜100 に正規化）で並べました。
...
![主要銘柄 GARPテーブル](img/01_PEG_ROE/02_majors_table.png){width="1200"}


実際の株価の動きと並べてみましょう（**直近 6 ヶ月**）。GARP スコア上位の**信越化（+58.0%）やリクルートＨＤ（+26.6%）、三井住友（+42.5%）は大きく上昇**、スコア下位の**任天堂（−43.7%）は下落**しており、「GARP マップ上の位置と直近の株価トレンドは緩く一致」しています。キーエンスは GARP スコアでは高 ROE グロース圏に分類されますが、直近 6 ヶ月は **+47.5%** と大きく上昇しており、GARP と値動きの一致度合いが確認できます。

各セル左下のドット色が GARP ゾーン（緑=理想/オレンジ=割高グロース/青=バリュー候補/赤=投資不適格）を表します。完全に綺麗な相関ではないものの、ファンダメンタルと値動きの方向は概ね一致しています。


<small style="color: var(--md-link-color);"><i class="fa-solid fa-expand"></i> クリックで拡大できます</small>

![主要銘柄チャート](img/01_PEG_ROE/04_majors_charts.png){width="1200"}
...
| 区分 | 該当銘柄 | コメント |
|---|---|---|
| ★GARP 理想 | 三井住友 | 唯一の左上、PEG 0.92 × ROE 10.4% |
...
- 母集団を **TOPIX 500（JPX 公式の規模区分）** に固定することで、選定根拠が外部リンク 1 本で検証できる。母集団 = TOPIX 500、中間層 = TOPIX 大型 100（Core30+Large70）、ラベル = 主要 15 社の 3 層構造で **割安 × 高 ROE の両立が大型株では稀** であることが一目で分かる
- 石油元売 3 社の比較では **コスモ（GARP 理想）が −5%、ROE が劣る ＥＮＥＯＳ が +30%** という GARP と株価の逆転現象が観察された。GARP は短期株価と必ずしも一致しない
- 石油元売の PEG / ROE は **会計利益ベース**。実態は在庫評価・原油市況・一時要因（のれん減損等）で大きく揺れるため、構造解釈と併せて読む必要がある
- ＥＮＥＯＳ の業績予想修正率は **基準次第で ▲94% から +4.76% まで分散**（編集部試算）。連載03〜05 のシグナル解釈は、この 4 基準のどれを採用するかで結論が変わる
...
*データ出典: 証券会社が無料で提供する ROE / 前期EPS / 予想EPS + yfinance 日足 Close*
