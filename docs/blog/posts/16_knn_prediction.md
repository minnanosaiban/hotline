...
連載15 では「丸紅 2026/3 期の類似 Top-15 が平均 CAR +2.43% だったのに、丸紅自身は -9.39% だった」という強い乖離を発見しました。本記事の連載16 ではこのアプローチを **232 銘柄全体に展開** し、機械学習の **K-Nearest Neighbors（K-NN）回帰** で「**類似決算群の CAR から自身の CAR を予測**」する実装を回します。
...
| データ | 件数 |
|---|---|
| 連載15 features.parquet（10 次元特徴量） | 287 銘柄（特徴量 ≥7 個揃う） |
...
つまり「**類似決算群の平均 CAR は、自身の CAR の予測値として使えない**」。連載15 で見た「丸紅 -9.39% vs 類似群 +2.43%」は **特殊事例ではなく一般的な構図** だったわけです。
...
一方、**ＥＮＥＯＳの誤差 −2.37pp は丸紅・双日より小さく**、shocks.csv の Top-30（誤差 ±10pp 以上）にも入りません。初動 [-1,+1] の +1.36% はポジティブだったものの、**5 営業日で −4.68% に反転**。これは連載12 で観察した「高 OPM セグメントの正常化局面」が後追いで市場に認識されたとも読めます。AI 予測フレームからは **「数値特徴量で概ね説明できる範囲」** ― ある意味「予測がほぼ機能した事例」にあたります。
...
ＥＮＥＯＳ の [−1, +5] は確定し **−4.68%** ― 初動 +1.36% が 5 日で反転した事実は、連載12 で観察した「高 OPM セグメントの正常化局面」と整合します。[−1, +20] が確定する 2026 年 6 月以降に再評価すれば、「セグメント情報を読まないと見えない、後追いネガショック」のシグナルとして更に評価できます。
...
K-NN の統計的有意性には **本来 1,000 サンプル以上** ほしい。287 銘柄は連載07 でカバーしている範囲の制約。連載15 で見た「業種クラスタリング」は出るが、予測精度として有意な数値は出にくい。
...
- narrative 主要銘柄の最終結論:
    ...
    - **ＥＮＥＯＳ [−1,+5]=−4.68%（誤差 −2.37pp）= AI 予測ほぼ的中の「軽いネガ」事例**。初動 [-1,+1] +1.36% の後 5 日で反転。営業利益急回復（4,666 億、公表 +25.5% / 継続事業ベース +339.8%）+ 在庫影響除き 4,744 億で 2025/3 期主張「実質 4,400 億維持」を上回って着地という数字パターンは市場に概ね織り込まれたが、高 OPM セグメント正常化局面の再認識が 5 日で後追い（連載01 4 基準試算・連載12 と整合）
