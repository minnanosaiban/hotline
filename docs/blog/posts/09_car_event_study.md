...
中央値は [-1,+5] で **-0.25%** に沈み、[-1,+20] で **+0.20%** に持ち直しています。

![ウィンドウ別サマリ](img/09_car_event_study/02_window_summary.png){width="1200"}

これは「**発表直後 1 週間で投資家の利食いが入り、その後ドリフト本体が出る**」というクラシックな PEAD パターンに合致します。勝率も同じく [-1,+5] で 48.7% と一時的に 50% を割ります。

### 短期 CAR × 長期 CAR の散布図 ― ドリフトは「初動の方向に続く」

短期 CAR と長期 CAR の **相関係数は r = +0.694**。決算直後 2 日の初動↑はその後のドリフト↑へ、初動↓は↓へと続き、PEAD の核心 **「初動の方向に株価が続く」** が散布図ではっきり出ます。
...
## narrative 5社で「個別決算の効き」を確認

### 平均 CAR ― TOPIX 超過 vs セクター超過

5 社を TOPIX 超過・セクター超過の二重 CAR で比べると、**ＥＮＥＯＳ が際立ちます ― セクター超過 +4.43%** と、エネルギー業界全体の動きを差し引いても個別決算で +4.4% 上乗せ。対照的にコスモエネは **セクター超過 −3.28%** で業界に劣後します。
...
イベントごとに見ると、連載 narrative の評価が CAR にそのまま表れます ― **ＥＮＥＯＳ は 4 連続プラスののち業績予想修正で −13.89%、丸紅は直近通期 +8.94% で「個別決算が効いた」** とはっきり読めます。
...
連載 01〜08 で下した評価は、本記事の CAR でおおむね裏付けられました ― **丸紅は「健全」評価どおり直近通期 CAR +8.94%、ＥＮＥＯＳ は 1 位評価どおり 4 連続プラスののち業績予想修正で −13.89%**。評価と市場反応の対応を一覧にします。
