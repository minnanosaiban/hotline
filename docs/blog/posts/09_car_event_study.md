...
| 連載 | 視点 | 株価との関係 |
|---|---|---|
| 01-05 | 証券会社の無料指標（PEG / ROE / マルチファクター） | 発表前の銘柄選定 |
| 03 | XBRL 取得・パース・スキーマ | データ基盤 |
| 06 | アクルーアル | 利益の質 |
| 07 | 三角検証 | 実績×ガイダンス×コンセンサスの整合 |
| 08 | セグメント発進力 | 事業ポートフォリオ |
| **09 CAR** | **発表後の株価反応** | **市場での答え合わせ** |

連載06〜08 で「健全」「強い」「ピークアウト」と評価したシグナルが **本当に市場で機能していたか** を、株価データで実証するのが本記事のゴールです。
...
- 連載01〜12 が **決算データの中身** だったのに対し、連載09 は **市場での答え合わせ**。連載03 で構築した自前パイプラインで **8,049 決算イベントの CAR を実証**
