...
![アクルーアル分析](img/08_accrual_analysis/00_thumbnail.png){width="1280"}
...
### 連載08 の早期警報を補う「利益の質」レンズ

連載08 で扱った進捗率 Z-score は、**「予想に対する実績の乖離」** を見るシグナルでした。しかし連載03 で見た ＥＮＥＯＳ 2022 ピークのように、**実績が良い決算でも「利益の中身が良いか」は別問題**です。
...
![ＥＮＥＯＳ 利益の質ハイライト](img/08_accrual_analysis/05_eneos_quality.png){width="1200"}
...
![石油元売3社 アクルーアル 7 年推移](img/08_accrual_analysis/01_oil_3_accrual_timeline.png){width="1200"}
...
![純利益 vs 営業CF 3 社対比](img/08_accrual_analysis/02_oil_3_ni_vs_cf.png){width="1200"}
...
![13 社 7 年平均アクルーアル ランキング](img/08_accrual_analysis/03_13_companies_rank.png){width="1200"}
...
![純利益×営業CF 散布図 91 サンプル](img/08_accrual_analysis/04_ni_cf_scatter.png){width="1200"}
...
実運用では、本記事の早期警報（連載08 進捗率 Z-score）と組み合わせて **「業績が伸びているがキャッシュで裏付けされていない銘柄」** を抽出するアプローチが現実的です。
...
連載06 で構築した独自スキーマのおかげで、計算ロジックは数行で済みます。
...
- 連載08 の進捗率は「予想 vs 実績」のシグナル。本記事の **アクルーアル比率は「純利益 vs 営業 CF」のシグナル** ― 業績モメンタム分析の最後のレンズ
...
連載04〜07（業績軸）→ 連載06（XBRL 基盤）→ 連載08〜09（進捗率 × 利益の質）と、**「業績の量と質を独立に検証する」フレームワーク** が完成しました。次回連載10 は **三角検証** に進みます。同じ会社の同じ指標について、**有報 XBRL（連結）× 決算短信 XBRL × 証券会社の無料データ** の 3 ソースを突き合わせ、データの信頼性を多角的に検証します。
